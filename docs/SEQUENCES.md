# warden-ai-sdk sequence diagrams

Five sequence diagrams cover the wire-level paths the SDK can take:
`wardenWrap` boot + structural client detection, the non-streaming
...
`WardenPending.resolve` polling loop, and the standalone
`inspectRealtimeFunctionCall` helper for the OpenAI Realtime WS
surface. A request decision-tree flowchart closes the file.

The SDK is a transparent client of `warden-lite`'s `POST /mcp` +
`GET /pending/{id}` surface — the diagrams treat the partner's model
client, `warden-lite`, the agent's tool loop, and (for Realtime) the
WS handler as the cross-process boundaries.

## 1. `wardenWrap` — boot, validate, detect, install nested Proxies

`wardenWrap` runs once per client. It validates the option bag,
...
sequenceDiagram
    ...
    participant Wrap as wardenWrap
    ...
    Caller->>Wrap: wardenWrap(client, { endpoint, token?, mode?, timeoutMs?, onVerdict?, onPolicyError?, fetch?, retry? })
    ...
    alt any check fails
        V-->>Caller: throw WardenConfigError
    end
    ...
    alt client.messages.create is function
        ...
        D-->>Caller: throw WardenConfigError — client must expose messages.create or chat.completions.create
    end
...
denies always produce the same `WardenDenied.toolName` deterministically.
...
sequenceDiagram
    ...
    participant L as warden-lite POST /mcp
    ...
    par parallel inspect every tool_use
        ...
        T->>T: read X-Warden-Correlation-Id header
        alt 200
            ...
            T->>T: corr = header ?? body.correlation_id (else throw WardenTransportError 202)
            ...
            T-->>Insp: throw WardenTransportError on terminal failure (enforce) OR catch (observe)
        end
    ...
    loop verdicts in submission order calls[i]
        ...
        alt enforce mode AND verdict.kind == 'deny'
            Insp-->>Caller: throw WardenDenied { toolName, reasons, reviewReasons, intentCategory, correlationId }
        else enforce AND verdict.kind == 'pending'
            Insp-->>Caller: throw WardenPending { toolName, correlationId, reviewReasons, pollOnce closure }
        else
    ...
    loop every upstream chunk
        ...
        loop every choiceIdx queued for inspection (BEFORE yielding the chunk)
            ...
            par parallel inspect every tool call in the choice
                ...
                T-->>Batch: WardenVerdict OR throw
            end
            loop verdicts in submission order
                ...
                alt observe mode AND inspectToolUse threw WardenTransportError
                    ...
                    Batch-->>Caller: throw WardenPending — same — closure carries pollOnce
                else
...
When enforce mode throws `WardenPending`, the partner catches it,
...
sequenceDiagram
    ...
    participant Pending as WardenPending.resolve
    participant Poll as pollPendingOnce (closure captured at throw time)
    participant L as warden-lite GET /pending/{id}
    ...
    loop while Date.now() < deadline
        ...
        alt 200
            ...
            alt view.decision == 'allow'
                ...
                Pending-->>Partner: throw WardenDenied { toolName, reasons: [decider_note OR 'operator denied'], reviewReasons, intentCategory: 'PendingDenied', correlationId }
            else view.decision == null
                Pending->>Pending: not decided yet — sleep min(pollIntervalMs, remaining)
            end
        else 401 or 404 (terminal)
            L-->>Poll: status
            Poll-->>Pending: throw WardenTransportError(status)
    ...
    Note over Partner: typical pattern:<br/>catch WardenPending → await e.resolve() → retry the original create() now that operator approved
```
...
for `wardenWrap` to intercept. Instead the SDK exposes a standalone
...
falling back to the raw string on parse failure so warden can still
...
sequenceDiagram
    ...
    L-->>T: 200 / 403 / 202 / 5xx
    T-->>Helper: WardenVerdict (no throw on deny — caller decides)
...
flowchart captures the final outcomes — pass-through, `WardenDenied`,
`WardenPending`, `WardenTransportError`, or a `WardenConfigError`
raised before any of those.

```mermaid
flowchart TD
    Start([wrapped.create args]) --> Cfg{validateOptions OK<br/>+ detectClient OK?}
    Cfg -->|no| Cerr[throw WardenConfigError<br/>caller bug — bad endpoint, missing client, etc.]
    ...
    Tport -->|enforce| Te[throw WardenTransportError<br/>fail-closed]
    ...
    Vk -->|deny| Dn[await onVerdict if set<br/>throw WardenDenied with reasons + intentCategory + correlationId]
    Vk -->|pending| Pn[await onVerdict if set<br/>throw WardenPending with closure-bound pollOnce]
    ...
    Rd -->|deny within deadline| Rdn[throw WardenDenied with PendingDenied intent + decider_note]
    Rto[throw WardenTransportError — not decided within timeoutMs]
    Rd -->|deadline trip| Rto
    Rd -->|401 or 404 terminal| Rterm[throw WardenTransportError immediately]
