...
- `delete_user` is denied by policy → `ClavenarDenied` is thrown. (red)
- `transfer_funds` parks for human review → `ClavenarPending` is thrown;
...
Prereq: clavenar-lite binary somewhere `start.sh` can find it. The
script looks at `$CLAVENAR_LITE_BIN`, then sibling
`../clavenar-lite/target/{release,debug}/clavenar-lite`, then `$PATH`. If
you're building from this workspace:

```sh
cd ../clavenar-lite && cargo build && cd -
```
...
[1/3] agent: "fetch user 42"
        ...
        clavenar:   [ALLOW] tool="fetch_user"
        result:   message returned, your tool-execution loop runs the tool

[2/3] agent: "delete user 42"
        tool_use: delete_user({"id":42})
        clavenar:   [DENY]  tool="delete_user" intent="Routine"
        result:   ClavenarDenied thrown — your existing throw-handler kicks in
...
[3/3] agent: "transfer $100 from acct-A to acct-B"
        ...
        clavenar:   [PEND]  tool="transfer_funds" corr="8f1d…"
                  reason: Review: transfer_funds requires human approval before execution.
                  awaiting operator decision …
        operator: $ clavenar-lite pending decide 8f1d… --allow --note 'demo auto-approve'
...
to the canonical operator command — `clavenar-lite pending decide` —
...
approval (configure `--slack-webhook-url` on `clavenar-lite start` for
one-way alerts) or a dashboard click; the SDK side is unchanged:
`catch (e instanceof ClavenarPending) { await e.resolve(); }`.
...
The `--cols 60` cap is deliberately tight — almost every clavenar output
...
`CLAVENAR_DEMO_PAUSE_MS` inserts a per-scenario reading pause so viewers
...
as **yellow** and parks: clavenar-lite responds `202 Accepted` with
`{status, correlation_id, review_reasons}` and stashes the request in
its `pendings` table.

SDK side, the wrap throws `ClavenarPending` carrying the correlation id
and a pre-bound poll closure. The demo runner schedules an
auto-approve `clavenar-lite pending decide` shell-out after 1.5s;
...
- `decision: "deny"`  → `resolve()` throws `ClavenarDenied` (same shape
  as the synchronous deny path, with the operator's note as the reason).
- Deadline elapsed    → `resolve()` throws `ClavenarTransportError`.
