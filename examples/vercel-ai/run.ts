/**
 * Vercel AI SDK + warden recipe — runs `generateText`, intercepts
 * the returned `toolCalls` array through warden before dispatching
 * to local tool handlers. Demonstrates the minimum-change path.
 *
 * Local tool implementations are stubbed — the only network call
 * out of process is to `warden-lite` (and to Anthropic, if you've
...
async function dispatchWithWarden(toolCalls: ToolCall[]) {
  for (const call of toolCalls) {
    console.log(`\n→ tool: ${call.toolName}`);
    try {
      // Inspect the tool call directly. inspectToolUse is the
      // lower-level surface used by wardenWrap internally; we use
      ...
      if (e instanceof WardenDenied) {
        ...
        console.log(`     operator decides via wardenctl pending decide.`);
      } else {
...
await dispatchWithWarden(fakeToolCalls());
